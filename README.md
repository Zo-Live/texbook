...
生成的 `.tex` 默认使用 `ctexart`、`amsmath`、`amsthm`、`amssymb`。如果需要在本地编译生成结果，请准备 TeX Live、`xelatex` 和 `latexmk`。
...
包源码位于 `src/latex_tools`，从仓库根目录以可编辑模式安装后即可使用 `latex-tools` 命令。
...
`LATEX_TOOLS_LLM_BASE_URL` 是可选项；使用默认 OpenAI SDK 地址时可以不设置。
...
```bash
uv run latex-tools extract "input/lecture.pdf"
```

单文件转换到指定文件：

```bash
uv run latex-tools extract "input/lecture.pdf" -o "output/lecture.tex"
```
...
```bash
uv run latex-tools extract "input/lecture.pdf" --pages 1,3-6 -o "output/partial.tex"
```

追加一次性转换要求：

```bash
uv run latex-tools extract "input/lecture.pdf" --pages 7 -o "output/formulas.tex" \
...
```bash
uv run latex-tools batch input/ -o output/
```
...
- `--cache-dir`：断点续传缓存目录，默认 `build/.latex_tools_cache/`。
